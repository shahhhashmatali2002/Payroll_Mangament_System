...
if not data:
    ...
    cursor.execute(
        '''create table DBES_EI(
            Employee_ID int identity(1,1) Not Null,
            Joining_Date date Not Null,
            Employee_Name varchar(255) Not Null,
            Employee_Image varchar(255) Not Null,
            Employee_Gmail varchar(255) Not Null,
            ...
            Constraint PK_DBES_EI Primary Key (Employee_ID)
            );
                )'''
    )
...
class DBES_NewEmployee:
    NE_Employee_ID: IntVar

    # Uploaded = Uploaded
    def __init__(self, DBES_New_Employee):
        # self.Employee_ID = Employee_ID
        self.DBES_New_Employee = DBES_New_Employee
        self.DBES_New_Employee.title('Daily Base Employee Salary')
        # self.DBES_New_Employee.geometry('1360x768+0+0')
        self.DBES_New_Employee.attributes("-fullscreen", True)
        self.DBES_New_Employee.resizable(0, 0)

        title = Label(self.DBES_New_Employee, text="DbEs AdD NeW EmPlOyeE", bd=4, font=("times new roman", 40), bg="gray",
                      fg="black", relief=GROOVE, justify=CENTER)
        title.pack(side=TOP, fill=X)
        # _| Manage Frame |__
        self.DBES_NE_Manage_Frame = Frame(self.DBES_New_Employee, bd=4, relief=RIDGE, bg="gray")
        self.DBES_NE_Manage_Frame.place(x=20, y=100, width=500, height=640)

        # _| Function for Entry placeholder removing |_

        def click1(*args):
            self.NE_EN.focus()
            if self.DBES_NE_Employee_name.get() == "Employee Name":
                self.NE_EN.delete(0, 'end')
                self.NE_EN.focus()

        def click2(*args):
            self.NE_E.focus()
            if self.DBES_NE_Email.get() == "XXXX@gmail.com":
                self.NE_E.delete(0, 'end')
                self.NE_E.focus()

        def click3(*args):
            # if self.DBES_NE_Contact.get() == '923000000000':
            self.NE_C.delete(0, 'end')

        def click4(*args):
            # if self.DBES_NE_CNIC.get() == "0000000000000":
            self.NE_nic.delete(0, 'end')

        def click5(*args):
            if self.DBES_NE_AGE.get() == "Enter AGE":
                self.DBES_NE_AGE.delete(0, END)
            # _| leave Function |__

        def leave(*args):
            self.DBES_NE_Manage_Frame.focus()

            if self.DBES_NE_Employee_name.get() == "":
                self.NE_EN.insert(0, 'Employee Name')
                self.DBES_NE_Manage_Frame.focus()
            elif self.DBES_NE_Email.get() == "":
                ...
                self.DBES_NE_Manage_Frame.focus()
            elif self.DBES_NE_AGE.get() == "":
                self.DBES_NE_AGE.insert(0, "Enter AGE")

        # -----} All Variable {-----
        self.DBES_NE_Employee_ID = IntVar()
        self.DBES_Date = StringVar()
        self.DBES_NE_Employee_name = StringVar()
        self.DBES_NE_Email = StringVar()
        self.DBES_NE_Contact = IntVar()
        self.DBES_NE_CNIC = IntVar()
        self.DBES_menu = StringVar()
        self.DBES_menu1 = StringVar()
        self.NE_Search_ID = IntVar()
        self.NE_Search_name = StringVar()
        self.DBES_NE_age = IntVar()
        self.DBES_imgpath = StringVar()
        ...
        self.upload_image = Frame(self.DBES_NE_Manage_Frame, width=200, height=200).place(x=270, y=20)

        self.NE_img_path = Entry(self.DBES_NE_Manage_Frame, bd=4, width=20, font=("times new roman", 15),
                                 textvariable=self.DBES_imgpath)
        self.NE_img_path.place(x=270, y=240)
        self.NE_img_path.insert(0, 'Image Path')
        self.NE_img_path.bind("<Button-1>", click6)
        Button(self.DBES_NE_Manage_Frame, text='UPLOAD IMAGE', width=15, font=("times new roman", 10, "bold"), relief=GROOVE,
        ...
        self.DBES_NE_Employee_ID = IntVar()
        self.NE_EID = Entry(self.DBES_NE_Manage_Frame, bd=4, width=20, font=("times new roman", 15),
                            textvariable=self.DBES_NE_Employee_ID)
        ...
        Label(self.DBES_NE_Manage_Frame, text="Date: ", bg="gray", fg="black",
              font=("times new roman", 20, "bold")).place(x=20, y=60)
        cal = DateEntry(self.DBES_NE_Manage_Frame, textvariable=self.DBES_Date, width=10, year=2021, month=1, day=1,state='readonly',
        ...
        self.NE_EN = Entry(self.DBES_NE_Manage_Frame, bd=4, width=20, font=("times new roman", 15),
                           textvariable=self.DBES_NE_Employee_name)
        ...
        self.NE_E = Entry(self.DBES_NE_Manage_Frame, bd=4, width=20, font=("times new roman", 15),
                          textvariable=self.DBES_NE_Email)
        # Add text in Entry box
        self.NE_E.insert(0, 'XXXX@gmail.com')
        ...
        def only_allow_numbers1(event):
            ...
            if char.isprintable() and (not event.char.isdigit()):
                ...
                self.DBES_New_Employee.focus()
                self.NE_nic.focus()
                return "break"

        # ----} Contact {----
        self.NE_C = Entry(self.DBES_NE_Manage_Frame, bd=4, width=20, font=("times new roman", 15),
                          textvariable=self.DBES_NE_Contact)
        ...
        self.NE_nic = Entry(self.DBES_NE_Manage_Frame, bd=4, width=20, font=("times new roman", 15),
                            textvariable=self.DBES_NE_CNIC)
        # Add text in Entry box
        ...
        # -----} City {------
        self.DBES_menu = StringVar()
        self.DBES_menu.set("Select City")

        # Create a dropdown Menu
        self.drop = OptionMenu(self.DBES_NE_Manage_Frame, self.DBES_menu, 'Karachi', 'Sindh', 'Lahore', 'Punjab', 'Islamabad',
                               'Peshawur')
        self.drop.place(x=20, y=362, width=120)

        # -----} Gender {------
        self.DBES_menu1 = StringVar()
        self.DBES_menu1.set("Select Gender")

        # Create a dropdown Menu
        self.drop = OptionMenu(self.DBES_NE_Manage_Frame, self.DBES_menu1, 'Male', 'Female', 'Others')
        self.drop.place(x=160, y=362, width=120)

        # ---} Employee Age {----
        self.DBES_NE_AGE = Entry(self.DBES_NE_Manage_Frame, bd=4, width=15, font=("times new roman", 15),
                            textvariable=self.DBES_NE_age)
        self.DBES_NE_AGE.place(x=300, y=362)
        self.DBES_NE_AGE.delete(0, END)
        self.DBES_NE_AGE.insert(0, "Enter AGE")
        self.DBES_NE_AGE.bind("<Enter>", click5)
        self.DBES_NE_AGE.bind("<Leave>", leave)

        # 6
        Label(self.DBES_NE_Manage_Frame, text="Full Address", font=("times new roman", 16, "bold"), bg="gray",
              fg='black').place(x=20, y=400)
        self.DBES_NE_Address = Text(self.DBES_NE_Manage_Frame, width=55, height=2)
        self.DBES_NE_Address.place(x=20, y=430)

        # __ | Entry Box End | __
        # __| Button |__

        self.NE_submit_btn = Button(self.DBES_NE_Manage_Frame, text="SUBMIT", width=15, font=("times new roman", 10, "bold"),
                                    relief=GROOVE, bd=4, bg='maroon', fg='oldlace', command=self.submit)
        self.NE_submit_btn.place(x=50, y=530)

        self.delete_btn = Button(self.DBES_NE_Manage_Frame, text="Delete", width=15, font=("times new roman", 10, "bold"),
                                 relief=GROOVE, bd=4,
                                 bg='red', fg='white', command=self.delete)
        self.delete_btn.place(x=180, y=530)
        self.delete_btn['state'] = DISABLED

        self.update_btn = Button(self.DBES_NE_Manage_Frame, text="UPDATE", width=15, font=("times new roman", 10, "bold"),
                                 relief=GROOVE, bd=4,
                                 bg='mistyrose', fg='black', command=self.update)
        self.update_btn.place(x=310, y=530)
        self.update_btn['state'] = DISABLED

        self.bacnk_btn = Button(self.DBES_NE_Manage_Frame, text="Back", width=15, font=("times new roman", 10, "bold"),
                                relief=GROOVE, bd=4,
                                bg='royalblue', fg='black')
        self.bacnk_btn.place(x=180, y=580)

        Button(self.DBES_NE_Manage_Frame, text="Close", width=15, height=1, font=("times new roman", 10, "bold"),
               relief=GROOVE, bd=4,
               bg='aliceblue', fg='crimson', command=lambda: exit()).place(x=310, y=580)

        self.clear_btn = Button(self.DBES_NE_Manage_Frame, text="Clear", width=15, font=("times new roman", 10, "bold"),
                                relief=GROOVE, bd=4,
                                bg='palegreen', fg='black', command=self.clear).place(x=50, y=580)

        # _| Detail Frame |__
        self.DBES_NE_Detail_Frame = Frame(DBES_New_Employee, bd=4, relief=RIDGE, bg="gray")
        self.DBES_NE_Detail_Frame.place(x=545, y=100, width=800, height=640)

        lbl_search = Label(self.DBES_NE_Detail_Frame, text="Search By", bg="gray", fg="black",
        ...
        def leave1(*args):
            self.DBES_NE_Detail_Frame.focus()
        ...
        def NE_Search_Id(*args):
            ...
            cursor.execute(
                "select * from Payroll_Management.dbo.DBES_EI where Employee_ID Like '%" +
        ...
        # ------} Search Entery {--------
        self.NE_Search_Id = Entry(self.DBES_NE_Detail_Frame, width=15, font=("times new roman", 13, "bold"), bd=5,
        ...
        def search(*args):
            ...
            cursor.execute(
                "select * from Payroll_Management.dbo.DBES_EI where Employee_Name Like '%" + str(
        ...
        # ------} Search Entery {--------
        self.NE_Search_Name = Entry(self.DBES_NE_Detail_Frame, width=15, font=("times new roman", 13, "bold"), bd=5,
        ...
        self.NE_Table_Frame = Frame(self.DBES_NE_Detail_Frame, bd=4, relief=RIDGE, bg="white")
        ...
        self.NE_Employee_table = ttk.Treeview(self.NE_Table_Frame, columns=(
            "Employee_ID", "Joining_Date", "Employee_Name","Employee_Image", "Gmail", "Phone_Number",
            "CNIC", "City", "Gender", "Employee_Age", "Full_Address"),
    ...
    def submit(self, *args):
        ...
        if self.DBES_NE_Employee_name.get() == 'Employee Name' or self.DBES_NE_Employee_name.get() == "":
            messagebox.showerror("Error", "Please Enter Employee Name")

        elif self.DBES_NE_Email.get() == 'XXXX@gmail.com' or self.DBES_NE_Email.get() == "":
        ...
        elif self.DBES_menu.get() == "Select City" or self.DBES_menu.get() == "":
            messagebox.showerror("Error", "Please Select City")

        elif self.DBES_menu1.get() == "Select Gender" or self.DBES_menu1.get() == "":
            messagebox.showerror("Error", "Please Select Gender")

        elif self.DBES_NE_AGE.get() == "Enter AGE" or self.DBES_NE_AGE.get() == "":
            messagebox.showerror("Error", "Please Enter AGE")

        elif len(self.DBES_NE_Address.get("1.0", "end-1c")) == 0:
            ...
            cursor.execute('''
                        ...
                        ''', (self.DBES_NE_Employee_name.get(), self.DBES_NE_Employee_name.get(), self.DBES_NE_Employee_name.get(),))
            data = cursor.fetchall()
            if not data:
                self.records = [self.DBES_Date.get(), self.DBES_NE_Employee_name.get(),self.DBES_imgpath.get(), self.DBES_NE_Email.get(),
                                self.DBES_NE_Contact.get(), self.DBES_NE_CNIC.get(), self.DBES_menu.get(), self.DBES_menu1.get(),
                                self.DBES_NE_AGE.get(),
                                self.DBES_NE_Address.get("1.0", "end-1c")]
                self.insert_query = "Insert into DBES_EI(Joining_Date,Employee_Name,Employee_Image,Employee_Gmail,Phone_Number,CNIC,City,Gender,Age,Full_Address) Values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?);"
                ...
                print("Date: " + self.DBES_Date.get())
                print("Employee Name: " + self.DBES_NE_Employee_name.get())
                print("EMAIL: " + self.DBES_NE_Email.get())
                print("Contact N0: " + self.NE_C.get())
                print("CNINC: " + self.NE_nic.get())
                print("City: " + self.DBES_menu.get())
                print("Gender: " + self.DBES_menu1.get())
                print("Full Address: " + self.DBES_NE_Address.get("1.0", "end-1c"))
                self.clear()
    ...
    def clear(self):
        ...
        self.DBES_NE_AGE.insert(0, 'Enter AGE')
        # _| Full Address |_
        self.DBES_NE_Address.delete(1.0, 'end')

        self.DBES_Date.set('1/1/21')
    ...
    def fetchdata(self):
        ...
        cursor.execute("select * from DBES_EI")
        rows = cursor.fetchall()
        if not rows:
            print("No data in database!")
        elif len(rows) != 0:
    ...
    def getcursor(self, ev):
        ...
        if not row:
            ...
            print(row)
            self.DBES_NE_Employee_ID.set(row[0])
            self.DBES_Date.set(row[1])
            self.DBES_NE_Employee_name.set(row[2])
            self.DBES_NE_Email.set(row[4])
            self.DBES_NE_Contact.set(row[5])
            self.DBES_NE_CNIC.set(row[6])
            self.DBES_menu.set(row[7])
            self.DBES_menu1.set(row[8])
            self.DBES_NE_age.set(row[9])
            self.DBES_NE_Address.delete("1.0", "end-1c")
            self.DBES_NE_Address.insert(END, row[10])

            # ----} Image Code {-----
            self.DBES_imgpath.set(row[3])
            lblimg = self.DBES_imgpath.get()
            try:
                print(lblimg)
    ...
    def update(self):
        if self.DBES_NE_Employee_name.get() == 'Employee Name' or self.DBES_NE_Employee_name.get() == "":
            ...
            cursor.execute(
                "update Payroll_Management.dbo.DBES_EI SET Joining_Date = ?, Employee_Name = ?,Employee_Image = ?, Employee_GMail = ?, Phone_Number = ?, CNIC = ?, City = ?, Gender = ?, Age = ?, Full_Address = ? where Employee_ID = ?",
                (
                    self.DBES_Date.get(),
                    self.DBES_NE_Employee_name.get(),
                    self.NE_img_path.get(),
                    self.DBES_NE_Email.get(),
                    self.DBES_NE_Contact.get(),
                    self.DBES_NE_CNIC.get(),
                    self.DBES_menu.get(),
                    self.DBES_menu1.get(),
                    self.DBES_NE_AGE.get(),
                    self.DBES_NE_Address.get("1.0", "end-1c"),
                    self.DBES_NE_Employee_ID.get(),
                ))
            connect.commit()
            self.fetchdata()
            self.clear()
    ...
    def delete(self):
        ...
        if MsgBox == 'yes':
            ...
            cursor.execute(
                "delete from Payroll_Management.dbo.DBES_EI  where Employee_ID = ?",
                (
                    self.DBES_NE_Employee_ID.get(),
                ))
            connect.commit()
...
# obj = DBES_NewEmployee(DBES_New_Employee)
